...
import { RecordData, RecordDataV1, RecordDataV2 } from '../ts-interfaces/record-data';
...
function isIdentifier(identifier): identifier is StableRecordIdentifier {
  return (
    typeof identifier === 'object' && identifier !== null && identifier[IS_IDENTIFIER] === true
  );
}
...
export function getRecordDataManagerFor(
  ...
  if (identifier === undefined && isVersion2RecordData(recordData)) {
    ...
    throw new Error(
      'Cannot lookup the RecordDataManager for a V2 RecordData without an identifier'
    );
  }
...
export class RecordDataManager {
  ...
  getRelationship(identifier: StableRecordIdentifier, propertyName: string, isCollection = false) {
    ...
    if (this.isDeprecated(recordData)) {
      ...
      return isBelongsTo
        ? recordData.getBelongsTo(propertyName)
        : recordData.getHasMany(propertyName);
    }
